...
def align_subswaths(data, regions, nswaths=3):
    """
    Align constant phase offsets between subswaths so they fall within [-pi, pi].

    Parameters
    ----------
    data : 2D np.ndarray
        Ionospheric phase array with NaNs outside valid region.
    regions : 2D np.ndarray
        Segmentation mask from multi-Otsu (values: 0..nswaths).
    nswaths : int
        Number of subswaths.

    Returns
    -------
    data_aligned : 2D np.ndarray
        Phase-corrected ionosphere.
    offsets : list
        Applied phase shift for each swath (rad).
    """
    data_aligned = data.copy()
    offsets = [0.0] * nswaths

    # Reference = first swath
    ref_mask = (regions == 0)
    ref_phase = np.nanmedian(data[ref_mask])

    for i in range(1, nswaths):
        mask_i = (regions == i)
        if not np.any(mask_i):
            continue
        phase_i = np.nanmedian(data[mask_i])

        # Difference relative to reference
        diff = phase_i - ref_phase

        # Apply correction
        data_aligned[mask_i] -= diff
        offsets[i] = diff

    return data_aligned, offsets
